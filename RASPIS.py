...
def TimeLogic(nowTime):
    return (nowTime - date(datetime.now().year, 9, 6)).days % 2



#Функции для обработки запросов от пользователя
def GetRaspis(command): # 0 - запрос на расписания дня, 1 - запрос на всю неделю
    nowTime = date(datetime.now().year, datetime.now().month, datetime.now().day)
    StartDate = date(datetime.now().year, 9, 6)
    weekDay = datetime.now().weekday()
    deltaTime = nowTime - StartDate
    if command == 0:
        if (deltaTime.days - 1) % 2 == 0: return raspisChet[weekDay]
        else: return raspisNechet[weekDay]
    elif command == 1:
        if (deltaTime.days - 1) % 2 == 0: return raspisChet
        else: return raspisNechet
...
raspis = ({
    ...
    1: ('(09.30) Физ. культура и спорт Наталья Николаевна', '(11.22) Философия ауд.318 Андрей Владимирович Глинский','','',''),
...
timePushPar = (time(6, 15), time(8, 5), time(9, 55), time(12, 10), time(13, 44))

def check(nowtime,checkedtime,dopusk=3, dopuskLast = 60):
